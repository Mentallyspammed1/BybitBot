...
Ah, a flicker in the matrix! Pyrmethus perceives a `KeyError`—a whisper from the digital void indicating a missing piece in your configuration spell. The trace points to `momentum_ma_long`, a vital component for calculating the mystical momentum of the market.

This error arises because the `whalebot_v1.py` script is attempting to access the `momentum_ma_long` setting from your `config.json` file, but it is not finding it there.  It's as if a crucial ingredient is missing from your potion!

To rectify this, we must ensure that your `config.json` file is correctly configured and contains all the necessary incantations, including `momentum_ma_long`.

Please replace the contents of your `config.json` file with the complete and corrected version below. This will ensure that all the necessary configuration parameters, including the missing `momentum_ma_long`, are present:
...
**To Update Your `config.json` in Termux:**

1.  Open Termux.
2.  Navigate to the directory where your `config.json` file is located (likely the same directory as `webwhale_v1.py`).
3.  Use a text editor like `nano` or `vim` to open `config.json`:
    ```bash
    nano config.json
    ```
    or
    ...
    vim config.json
    ```
4.  **Carefully replace all the existing content** of `config.json` with the JSON code provided above.
5.  If using `nano`, press `Ctrl+X`, then `Y` to save and exit. If using `vim`, press `Esc`, then type `:wq` and press `Enter`.

**After updating `config.json`, please run `webwhale_v1.py` again.** The `KeyError: 'momentum_ma_long'` should be resolved, and your bot should commence its real-time analysis as intended.

**Important Notes:**

*   **Verify Correct Placement:** Ensure `momentum_ma_long` is at the **top level** of the JSON structure, alongside `interval`, `analysis_interval`, `indicators`, etc., and **not** nested within `signal_config` or another section.
*   **JSON Syntax:** Double-check that your `config.json` is valid JSON.  Even a small typo (missing comma, extra quote) can cause parsing errors. If you are unsure, you can use an online JSON validator to check your `config.json` after editing.

By ensuring your `config.json` contains the complete and correct configuration, you will empower your bot to perform its mystical market analysis without hindrance. Let Pyrmethus know if the shadows persist, and we shall delve deeper into the digital ether!
Greetings, Pyrmethus! It sounds like you've pinpointed the source of the digital hiccup perfectly – a missing momentum_ma_long indeed! You're right, it's like a missing ingredient throwing off the entire alchemical formula of your market analysis bot.

The config.json you've provided looks spot-on! It includes the crucial momentum_ma_long parameter right where it should be, alongside its fellow configuration spells like interval, analysis_interval, and all the indicator settings. Excellent diagnosis!

Let's reinforce the steps to update your config.json within Termux, just to make sure the incantation is performed flawlessly:

Venture into Termux: Open your Termux application, the gateway to your command-line realm.
Navigate to the Script's Lair: Use the cd command to journey to the directory where whalebot_v1.py and config.json reside. For example, if they are in your home directory, you might just need to type cd and press Enter. If they are in a subdirectory, use cd path/to/your/directory.
Summon the Text Editor: Choose your preferred text editor. nano is a simpler option, while vim is more powerful but has a steeper learning curve.
For nano: Type nano config.json and press Enter.
For vim: Type vim config.json and press Enter.
Replace the Old with the New: Carefully select and delete all the current content of your config.json file within the editor. Then, paste the complete JSON code you provided into the editor. Double-check that you've replaced everything and that you've pasted the entire new configuration.
Seal the Changes:
In nano: Press Ctrl + X. When prompted "Save modified buffer?", type Y (for Yes) and press Enter.
In vim: Press Esc to exit insert mode. Then type :wq (for Write and Quit) and press Enter.
Invoke the Bot Anew: Now that the config.json is updated with the missing ingredient, run your webwhale_v1.py script again. The KeyError should vanish like mist in the morning sun!
A Word of Caution, Wise Pyrmethus:

Placement is Key: As you correctly noted, ensure momentum_ma_long is at the top level of the JSON structure, not nestled within any sub-sections. It needs to be a primary spell component!
JSON Purity: JSON is a precise language. A misplaced comma, a missing quote, or an extra character can break the enchantment. If doubt lingers, copy the modified config.json and paste it into an online JSON validator (search for "JSON validator" on the web) to ensure it's structurally sound.
Once you've performed this configuration ritual, run webwhale_v1.py once more. With the momentum_ma_long incantation now present, the bot should hopefully awaken and begin its analysis without issue.

Let me know if the digital shadows recede and the bot springs to life! If the error persists, we shall delve deeper into the mystical code and uncover any further secrets it may hold. Good luck, Pyrmethus!
...
async def analyze_market_data_signals(indicators_df: Dict[str, Union[pd.Series, pd.DataFrame]], support_resistance: dict, orderbook: Optional[dict], config: dict, df: pd.DataFrame, current_price: float) -> Optional[dict]:
    ...
    active_indicators = {ind: w for ind, w in weights.items() if config["indicators"].get(ind, False).get("enabled", False)}
...
async def format_signal_output(signal: Optional[dict], indicators: dict, indicators_df: Dict[str, Union[pd.Series, pd.DataFrame]], current_price: float, logger: logging.Logger, last_output_time: float) -> float:
    ...
    if signal:
        ...
        try:
            async with aiofiles.open(os.path.join(LOG_DIRECTORY, output_config["alert_file"]), "a") as f:
...
def vwap_status_logic(vwap_series: pd.Series, current_price: float) -> Tuple[str]:
    ...
    return ("[yellow]Neutral[/yellow]",) if pd.isna(vwap) or pd.isna(current_price) else (f"[{'bullish' if current_price > vwap else 'bearish' if current_price < vwap else 'neutral'}]{'Bullish' if current_price > vwap else 'Bearish' if current_price < vwap else 'Neutral'}[/]",)
...
class TradingAnalyzer:
    ...
    def calculate_indicators(self):
        self.indicator_values = {
            "ema": self.calculate_ema(self.config["signal_config"]["ema_period"]),
            "mom": self.calculate_momentum(),
            "obv": self.calculate_obv(),
            "rsi": self.calculate_rsi(self.config["rsi_period"]),
            "stoch_rsi_k": self.calculate_stoch_rsi()["k"],
        ...
        self.indicator_values["bollinger_bands_df"] = self.calculate_bollinger_bands()
    ...
    def calculate_stoch_rsi(self) -> pd.DataFrame:
        rsi = self.calculate_rsi(self.config["signal_config"]["stoch_rsi_period"])
    ...
    def calculate_bollinger_bands(self) -> pd.DataFrame:
        ...
        upper_band = sma + (std * self.config["bollinger_bands_std_dev"])
        lower_band = sma - (std * self.config["bollinger_bands_std_dev"])
...
Analyze the code, enhance it, optimize it, return complete improved code. 
import os
import logging
import pandas as pd
import numpy as np
...
if __name__ == "__main__":
    asyncio.run(main())
Enhancements and Optimizations Made:

Configuration Loading and Validation:

Improved load_config function with more robust error handling for JSON parsing and file reading.
Added a _validate_config helper function to centralize and clarify configuration validation logic.
Included more specific checks for interval and orderbook_limit validity with console warnings and default value fallback.
Saved corrected configuration back to config.json to persist fixes.
Error Handling and Logging:

Enhanced error handling in _bybit_api_request to catch aiohttp.ClientError (for network issues) and asyncio.TimeoutError.
Added more detailed error messages in API request functions, including attempt numbers and specific error types.
Improved WebSocket error handling, especially for subscription failures and invalid symbols (now raises ValueError to stop processing invalid symbols).
Added try-except blocks around WebSocket message processing in process_websocket_message to prevent bot crashes from unexpected message formats.
Refined logging messages throughout the code for better clarity and debugging.
Code Structure and Readability:

Refactored large functions into smaller, more modular functions (e.g., WebSocket subscription and message processing are now in separate functions).
Created helper functions (_output_signal_to_console, _save_signal_to_files, _output_indicators_to_console, _add_indicator_row_to_table, _determine_indicator_status) to break down format_signal_output for better organization and readability.
Improved docstrings for functions and classes to explain their purpose and usage.
Used more descriptive variable names.
Added comments to clarify complex logic sections.
Performance and Efficiency:

Minor optimization: In TradingAnalyzer.update_data, kept only the tail of the DataFrame (tail(200)) to limit memory usage and calculation overhead for very long-running bots.
Ensured efficient Pandas operations are used throughout the indicator calculations. Asynchronous operations were already well-utilized in the original code.
Code Clarity and Maintainability:

Centralized indicator status logic into _determine_indicator_status for consistency and easier modification.
Improved the structure of the TradingAnalyzer class, making indicator calculations internal methods (prefixed with _).
Removed duplicate ind_table.add_column calls in _output_indicators_to_console.
Robustness:

Added a check for empty initial klines data in websocket_stream to prevent the bot from starting without data.
Improved WebSocket reconnection logic and error handling to make the bot more resilient to network interruptions.
Alert Logging:

Simplified alert logging by using synchronous file I/O for the alert file to avoid unnecessary async complexity for simple alert messages.
These enhancements focus on making the code more robust, readable, maintainable, and slightly more efficient without drastically changing its core functionality. The improved error handling and logging will be particularly helpful in diagnosing issues and ensuring the bot runs reliably.
