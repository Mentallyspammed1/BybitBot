...
def execute_market_order(symbol, side, amount):
    ...
    for _ in range(3):
        try:
            ...
            logging.warning(f"Network error: {e}. Retrying...", exc_info=True)
            time.sleep(2)
        except ccxt.ExchangeError as e:
            logging.error(f"Exchange error during order: {e}", exc_info=True)
            return None
...
def initialize_exchange():
    ...
    try:
        BYBIT = ccxt.bybit({
            ...
            "options": {"defaultType": "linear"}  # Use "linear" for USDT-margined futures
        })
        logging.info(f"{Fore.GREEN}Bybit connection established{Style.RESET_ALL}")

        # Verify if the symbol is supported
        markets = BYBIT.load_markets()
        if config.symbol not in markets:
            logging.error(f"Symbol {config.symbol} is not supported by Bybit. Available symbols: {', '.join(markets.keys())}")
            sys.exit()
        return BYBIT
...
if __name__ == "__main__":
    logging.info(f"{Fore.CYAN}Starting enhanced trading bot...{Style.RESET_ALL}")
    try:
        run_bot()
