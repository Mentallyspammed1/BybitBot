    ...
    def compute_trade_signal_score(self, price, ema, rsi, orderbook_imbalance):
        ...
        if stoch_rsi_k is not None and stoch_rsi_d is not None:
            if stoch_rsi_k < 20 and stoch_rsi_d < 20:
                score += 1
                reasons.append("Stochastic RSI below 20 (bullish potential).")
            elif stoch_rsi_k > 80 and stoch_rsi_d > 80:
                score -= 1
                reasons.append("Stochastic RSI above 80 (bearish potential).")
            else:
    ...
    def place_order(self, side, order_size):
        try:
            if self.simulation_mode:
                ...
                logger.info(f"{Fore.CYAN}[SIMULATION] {side.upper()} order of size {order_size:.2f} executed at price {trade_details['price']} at time {trade_details['timestamp']}.{Style.RESET_ALL}")
