...
class ScalpingBot:
    ...
    def validate_config(self):
        ...
        risk_params = [
            ...
            ('time_based_exit_minutes', int, 1, None), ('trailing_stop_loss_percentage', (int, float), 0, 1)
        ]
    ...
    def place_order(self, side, order_size, confidence_level, order_type="market", price=None, stop_loss_price=None,
            ...
            if self.simulation_mode:
                ...
                if order_type == "market":
                    ...
                    if not stop_loss_price:
                        ...
                        return None
                    order = self.exchange.create_market_order(self.symbol, side, order_size, params=params) # Bybit doesn't directly support stop-market for TP/SL in createOrder, needs conditional order
                else:
        ...
        for position in list(self.open_positions):
            ...
            if position['side'] == 'buy':
                if 'trailing_stop_loss' in position:
                    position['trailing_stop_loss'] = max(position['trailing_stop_loss'],
                ...
                if current_price <= stop_loss_price:
                    ...
                    self.open_positions.remove(position)
                        ...
                        current_price >= position['entry_price'] * (1 + 2 * self.base_take_profit_pct)):
                ...
                if 'trailing_stop_loss' in position:
                    position['trailing_stop_loss'] = min(position['trailing_stop_loss'],
                                                           current_price * (1 + self.trailing_stop_loss_percentage))
                    stop_loss_price = position['trailing_stop_loss']
                else:
                    stop_loss_price = position['stop_loss']
            ...
            if len(self.open_positions) < self.max_open_positions:
                if signal_score >= 2:
                    ...
                    entry_order = self.place_order(
                        'buy', order_size, signal_score, order_type=self.entry_order_type,
                    ...
                    entry_order = self.place_order(
                        'sell', order_size, signal_score, order_type=self.entry_order_type,
                        price=limit_price if self.entry_order_type == 'limit' else None,
                        stop_loss_price=stop_loss_price, take_profit_price=take_profit_price
                    )
