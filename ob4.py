...
# import numpy as np # No longer strictly needed for calculations, keep if used elsewhere
...
CONFIG = {
    ...
    "VOLUME_THRESHOLDS": {'high': 10, 'medium': 2},
    "REFRESH_INTERVAL": 9,
    ...
    "STOCH_K_PERIOD": 14,
    "STOCH_D_PERIOD": 3,
    "STOCH_RSI_OVERSOLD": decimal.Decimal('20'), # Use Decimal for comparison
    "STOCH_RSI_OVERBOUGHT": decimal.Decimal('80'), # Use Decimal for comparison
    # Display Config
    "PIVOT_TIMEFRAME": '1d',
    "PNL_PRECISION": 2,
    "MIN_PRICE_DISPLAY_PRECISION": 2,
    "STOCH_RSI_DISPLAY_PRECISION": 2,
}
...
def format_decimal(value, reported_precision, min_display_precision=None):
    """Formats a decimal value to string, respecting reported precision but ensuring a minimum display precision if specified."""
    ...
    try:
        ...
        if min_display_precision is not None: display_precision = max(display_precision, int(min_display_precision))
        ...
        else: formatted_str = str(rounded_value.normalize())
        if int(reported_precision) == 0 and display_precision > 0 and '.' not in formatted_str: formatted_str += '.' + '0' * display_precision
...
def get_market_info(exchange, symbol):
    ...
    try:
        ...
        price_prec_raw, amount_prec_raw, min_amount_raw = market.get('precision', {}).get('price'), market.get('precision', {}).get('amount'), market.get('limits', {}).get('amount', {}).get('min')
...
def calculate_rsi_manual(close_prices_list, period=14):
    """Calculates RSI manually using Wilder's Smoothing."""
    ...
    rsi_values = [decimal.Decimal('NaN')] * period # Fill initial period with NaN or skip
    ...
    alpha = decimal.Decimal('1') / decimal.Decimal(period)
    for i in range(period, len(gains)):
        ...
        # avg_gain = alpha * gains[i] + (1 - alpha) * avg_gain # Alternative EMA calculation
        # avg_loss = alpha * losses[i] + (1 - alpha) * avg_loss # Alternative EMA calculation
    ...
    return rsi_values # Return list of RSI values
...
def calculate_stoch_rsi_manual(rsi_values, k_period=14, d_period=3):
    ...
    if not rsi_values or len(rsi_values) < k_period + d_period -1 : # Need enough RSI for K and D calc
         return None, None

    # Filter out potential NaNs from the start of RSI calculation
    valid_rsi = [r for r in rsi_values if r is not None and r.is_finite()]
    if len(valid_rsi) < k_period + d_period -1 :
         return None, None
    ...
    for i in range(k_period - 1, len(valid_rsi)):
        ...
        if max_rsi == min_rsi: # Avoid division by zero if RSI hasn't changed
            stoch_k = decimal.Decimal('50') # Or 0 or 100 depending on preference
        else:
            stoch_k = ((current_rsi - min_rsi) / (max_rsi - min_rsi)) * 100
        stoch_k_values.append(stoch_k)

    if not stoch_k_values or len(stoch_k_values) < d_period:
        return stoch_k_values[-1] if stoch_k_values else None, None # Return latest K if possible, D needs more data
...
def fetch_market_data(exchange, symbol, config):
    ...
    rate_limit_wait, network_wait = config["RETRY_DELAY_RATE_LIMIT"], config["RETRY_DELAY_NETWORK_ERROR"]

    # History needed: Max of SMA period or (RSI period + Stoch K period + Stoch D period) for calculations
    indicator_history_needed = max(config['SMA_PERIOD'], config['RSI_PERIOD'] + config['STOCH_K_PERIOD'] + config['STOCH_D_PERIOD']) + 5
    ...
    for call in api_calls:
        try:
            ...
            if call["desc"] == "positions": results[call["desc"]] = [p for p in results[call["desc"]] if p.get('symbol') == symbol and p.get('contracts') is not None and float(p['contracts']) != 0]
...
def display_combined_analysis(analyzed_orderbook, market_info, ticker_info, indicators_info, position_info, pivots_info, config):
    ...
    price_prec, amount_prec = market_info['price_precision'], market_info['amount_precision']
    pnl_prec, min_disp_prec, stoch_disp_prec = config["PNL_PRECISION"], config["MIN_PRICE_DISPLAY_PRECISION"], config["STOCH_RSI_DISPLAY_PRECISION"]
    ...
    if indicators_info.get('sma') is not None and last_price is not None:
        sma_val = indicators_info['sma']; sma_str_fmt = format_decimal(sma_val, price_prec, min_disp_prec)
    ...
    stoch_k_val, stoch_d_val = indicators_info.get('stoch_k'), indicators_info.get('stoch_d')
    ...
    if stoch_k_val is not None and stoch_d_val is not None:
        stoch_k_str, stoch_d_str = format_decimal(stoch_k_val, stoch_disp_prec), format_decimal(stoch_d_val, stoch_disp_prec)
        # Use Decimal for comparison
        if stoch_k_val < config['STOCH_RSI_OVERSOLD'] and stoch_d_val < config['STOCH_RSI_OVERSOLD']: stoch_color, stoch_signal = Fore.GREEN, "(Oversold)"
        elif stoch_k_val > config['STOCH_RSI_OVERBOUGHT'] and stoch_d_val > config['STOCH_RSI_OVERBOUGHT']: stoch_color, stoch_signal = Fore.RED, "(Overbought)"
        elif stoch_k_val > stoch_d_val: stoch_color = Fore.LIGHTGREEN_EX
        elif stoch_k_val < stoch_d_val: stoch_color = Fore.LIGHTRED_EX
    elif indicators_info.get('stoch_error'): stoch_k_str, stoch_d_str = "Error", "Error"; stoch_color = Fore.YELLOW

    print_color(f"  Stoch RSI ({config['RSI_PERIOD']},{config['STOCH_K_PERIOD']},{config['STOCH_D_PERIOD']}@{config['INDICATOR_TIMEFRAME']}): {stoch_color}%K={stoch_k_str}, %D={stoch_d_str} {stoch_signal}{Style.RESET_ALL}")
    ...
    if position_info.get('has_position'):
        ...
        pnl = position_info.get('unrealizedPnl', decimal.Decimal('0')); size_str = format_decimal(size, amount_prec)
        entry_str = format_decimal(entry_price, price_prec, min_disp_prec); pnl_val_str = format_decimal(pnl, pnl_prec)
    ...
    if pivots_info:
        ...
        for level in ['R3', 'R2', 'R1', 'PP', 'S1', 'S2', 'S3']:
            ...
            if value is not None:
                level_str = f"{level}:".ljust(4); value_str = format_decimal(value, price_prec, min_disp_prec)
                if isinstance(value_str, str): value_str = value_str.rjust(pivot_width)
                else: value_str = str(value).rjust(pivot_width)
    ...
    if is_placeholder_ob: print_color(f"  {Fore.YELLOW}Order book data unavailable.{Style.RESET_ALL}")
        ...
        print_color(f"\n--- Spread: {format_decimal(spread, price_prec, min_disp_prec)} ---", color=Fore.MAGENTA, style=Style.DIM)

        print_color("\n--- Volume Analysis ---", color=Fore.BLUE)
        print_color(f"  Total Ask: {Fore.RED}{format_decimal(analyzed_orderbook['ask_total_volume'], amount_prec)}{Style.RESET_ALL} | Total Bid: {Fore.GREEN}{format_decimal(analyzed_orderbook['bid_total_volume'], amount_prec)}{Style.RESET_ALL}")
...
def main():
    ...
    while True:
        ...
        try:
            ...
            # --- Process Indicators (Manual) ---
            indicators_info = {'sma': None, 'stoch_k': None, 'stoch_d': None, 'sma_error': False, 'stoch_error': False}
            if indicator_ohlcv:
                # Index 4 is close price in ccxt OHLCV structure
                close_prices = [candle[4] for candle in indicator_ohlcv]
                # SMA
                ...
                # Stoch RSI (Manual)
                rsi_list = calculate_rsi_manual(close_prices, CONFIG["RSI_PERIOD"])
                if rsi_list:
                    stoch_k, stoch_d = calculate_stoch_rsi_manual(rsi_list, CONFIG["STOCH_K_PERIOD"], CONFIG["STOCH_D_PERIOD"])
                    if stoch_k is not None: indicators_info['stoch_k'] = stoch_k
                    if stoch_d is not None: indicators_info['stoch_d'] = stoch_d
                    if stoch_k is None or stoch_d is None: indicators_info['stoch_error'] = True
                else: indicators_info['stoch_error'] = True # RSI calc failed
            else:
